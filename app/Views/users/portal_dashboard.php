...
<nav class="navbar navbar-dark navbar-warm shadow-sm mb-4">
    <div class="container">
        ...
        <div class="d-flex align-items-center">
            ...
            <a href="<?= base_url('r/logout') ?>" class="btn btn-sm btn-outline-light rounded-pill px-3">تسجيل الخروج</a>
        </div>
    </div>
</nav>
                        ...
                        <tbody>
                            <?php if (!empty($dependents)): foreach ($dependents as $dep): ?>
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <form action="<?= base_url('r/remove-member/' . $dep['id']) ?>" method="POST" onsubmit="return confirm('هل أنت متأكد من إزالة هذا الفرد من سجل العائلة النشط؟');" class="d-inline">
                                            <?= csrf_field() ?>
...
<div class="modal fade" id="addMemberModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow rounded-4">
            ...
            <form action="<?= base_url('r/add-member') ?>" method="POST">
                <?= csrf_field() ?>
